...
contract Arena is IArena, OwnableUpgradeable {
    ...
    struct BetInfo {
        ...
        address cardOwner;
        uint256 betId;
        // The coefficient to desired choice given by external provider, as a numerator for 10000.
        // This is NOT THE COEFFICIENT provided to the players, like 1.3 etc. This coeff shows
        // how disappropriate the probabilities of the outcome. If outcomes have the same likelyhood,
        // so the coefficients are somewhat around 2.0 : 2.0, this coeff = 1.
    ...
    function makeBetsGasFree(
        ...
        uint24[] memory coeffs = new uint24[](eventIds.length);
    ...
    function _makeBetsGasFree(
        ...
        uint256[] calldata cardIds,
        MatchResult[] calldata choiceIds,
        uint24[] memory coeffs,
        address caller,
        uint8 _v,
        bytes32 _r,
        bytes32 _s
    ) internal {
        // Important: coeffs are not used in users signature. So we don't need to validate them.
        ...
        for (uint256 i = 0; i < eventIds.length; ++i) {
            ...
            _makeBetCore(eventIds[i], cardIds[i], choiceIds[i], signer, coeffs[i]);
        }
    }
