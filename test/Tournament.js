...
describe("Tournament Tests", () => {
  async function deployContracts() {
    ...
    const tournament = await upgrades.deployProxy(Tournament, [
      mainCardToken.address,
      card.address,
    ]);

    await card.grantRole(await card.MINTER_ROLE(), manager.address);
    await tournament.grantRole(
      await tournament.TOURNAMENT_MANAGER_ROLE(),
      manager.address,
    );
    await mainCardToken.grantRole(
      await mainCardToken.MINTER_ROLE(),
      manager.address,
    );
    ...
    await mainCardToken
      .connect(manager)
      .mint(alice.address, 100_000_000_000_000_000_000n);
    ...
    const sendCardsToTournament = async (tournamentId, fee, cardIds) => {
      ...
      const tokenPermitSignature = ethers.utils.splitSignature(
        await alice.signMessage(perminMainTokensMsg),
      );
      ...
      const placeCardsSignature = ethers.utils.splitSignature(
        await alice.signMessage(placeCardsMsg),
      );
      ...
      await mainCardToken
        ...
        .permit(
          ...
          alice.address,
          fee,
          tokenPermitSignature.v,
          tokenPermitSignature.r,
          tokenPermitSignature.s,
        );
      await tournament
        .connect(backend)
        .registerForTournamentGasFree(
          cardIds,
          tournamentId,
          alice.address,
          placeCardsSignature.v,
          placeCardsSignature.r,
          placeCardsSignature.s,
        );
    };
