...
describe("Auction tests", () => {
  ...
  let mintNewCard,
    placeCard,
    placeCardToMaticAuction,
    takeCard,
    takeCardFromMaticAuction,
    placeBet,
    placeBetToMaticAuction;
  ...
  before(async () => {
    ...
    mintNewCard = async (to) => {
      const buyTx = await card
        .connect(to)
        .mint(Regular, { value: "10000000000000000000" });
    ...
    placeBetToMaticAuction = async (cardId, signer, price) => {
      await maticAuction
        .connect(signer)
        .placeBet(cardId, price, { value: price });
    };
  ...
  it("Alice can make a bet on Bobs card", async () => {
    ...
    expect(
      bobsBalanceAfterTakingCard.sub(bobsBalanceBeforeTakingCard).toString(),
    ).to.be.equal("118");
    const cardOwnerAfterTakingFromAuc = await card.ownerOf(cardId);
  ...
  it("Testing Auction with Matic with decimals", async () => {
    ...
    await expect(
      placeBetToMaticAuction(cardId, alice, ethers.utils.parseEther("0.05")),
    ).to.be.revertedWith("TooFew");

    const aliceBalanceBeforeBet = await alice.provider.getBalance(
      alice.address,
    );
    await placeBetToMaticAuction(
      cardId,
      alice,
  ...
  it("Supports gasless ops", async () => {
    ...
    await card
      ...
      ](bob.address, gaslessMan.address, cardId);
    ...
    const sendToAuctionMessage = ethers.utils.arrayify(
      ethers.utils.keccak256(
        ethers.utils.solidityPack(
          ...
          [cardId, "10", await auction.gasFreeOpCounter(gaslessMan.address)],
        ),
      ),
    );
    const signatureInfo = ethers.utils.splitSignature(
      await gaslessMan.signMessage(sendToAuctionMessage),
    );
  ...
  it("Old auction supports gasless ops", async () => {
    ...
    const sendToAuctionMessage = ethers.utils.arrayify(
      ethers.utils.keccak256(
        ethers.utils.solidityPack(
          ["uint256", "uint256", "uint256"],
          [cardId, "10", await oldAuction.gasFreeOpCounter(gaslessMan.address)],
        ),
      ),
    );
    const signatureInfo = ethers.utils.splitSignature(
