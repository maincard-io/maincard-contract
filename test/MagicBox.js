...
describe("MagicBox tests", function () {
  before(async () => {
    ...
    const subId = subIdEffects.events.filter(
      (e) => e.event === "SubscriptionCreated",
    )[0].args[0];
    await this.vrfCoordinatorMock.fundSubscription(
      subId,
    ...
    this.magicBox = await MagicBox.deploy(
      subId,
    ...
    this.sendRequestToBuyCard = async (rarity, value) => {
      ...
      const logs = effects.logs.filter(
        (e) =>
          e.topics[0] ===
    ...
    this.fulfillRequest = async (requestId, randomWord) => {
      const tx = await this.vrfCoordinatorMock.fulfillRandomWordsWithOverride(
        requestId,
        this.magicBox.address,
        [randomWord],
      );
      const effects = await tx.wait();
      const fulfillLogs = effects.events.filter(
        (e) => e.event === "RandomWordsFulfilled",
      );
      ...
      const iface = new ethers.utils.Interface(abi);
      const transferLogs = effects.events
        .filter(
          (e) =>
  ...
  it("Test unlucky", async () => {
    const requestId = await this.sendRequestToBuyCard(
      0,
  ...
  it("Test not enough funds", async () => {
    await expect(this.sendRequestToBuyCard(0, "10100000000000000000")).to.be
      .reverted;
  });
