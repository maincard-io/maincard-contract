...
describe("TournamentMatic", function () {
    async function deployContracts() {
        ...
        const instance = await upgrades.deployProxy(TournamentMatic, [card.address]);
        ...
        const createCard = async (rarity) => {
            ...
            const transferEventsForCreateTx = createCardTxReceipt.events.filter(e => e.event === "Transfer");
        ...
        const createTournament = async (tournamentId, fee, minRequiredRarity, requiredCardsAmount) => {
            await instance.connect(manager).setTournamentRules(tournamentId, fee, minRequiredRarity, requiredCardsAmount);
        }

        const participate = async (tournamentId, cardIds, value) => {
            await instance.connect(alice).registerForTournament(cardIds, tournamentId, { value });
        }

        const payout = async (tournamentId, winners, amounts) => {
            await instance.connect(manager).completeTournament(tournamentId, winners, amounts);
        }
        
        return { instance, card, cardBasic1Id, cardBasic2Id, owner, alice, bob, charlie, 
                 cardUncommon1Id, cardUncommon2Id,
                 createTournament, participate, payout };
    }
    ...
    it("Should not allow to create a tournament to wrong person", async () => {
        ...
        await expect(instance.connect(bob).setTournamentRules(1, 100, 0, 1)).to.be.revertedWith("msg.sender should have granted TOURNAMENT_MANAGER_ROLE");
    });

    it("Should not allow to participate in a tournament if not enough cards, not enough value", async () => {
        const { createTournament, participate, cardBasic1Id, cardBasic2Id } = await loadFixture(deployContracts);
        await createTournament(2, "1000000000000000000", 0, 2);

        await expect(participate(2, [cardBasic1Id], "1000000000000000000")).to.be.revertedWith("Tournament: wrong cards amount");
        await expect(participate(2, [cardBasic1Id, cardBasic2Id], "500000000000000000")).to.be.revertedWith("Tournament: wrong fee");
    ...
    it("Does not allow to participate in a tournament if not enough cards of required rarity", async () => {
        ...
        await expect(participate(3, [cardBasic1Id, cardBasic2Id], "1000000000000000000")).to.be.revertedWith("Tournament: wrong rarity");
    });

    it("Does allow to participate if rarity is higher", async () => {
        const { createTournament, participate, cardUncommon2Id, cardUncommon1Id } = await loadFixture(deployContracts);
    ...
    it("Does not allow to participate to more cards", async () => {
        ...
        await expect(participate(5, [cardUncommon1Id, cardUncommon2Id], "1000000000000000000")).to.be.revertedWith("Tournament: wrong cards amount");
    });

    it("Does not allow to participate to the same card twice", async () => {
        const { createTournament, participate, cardUncommon2Id } = await loadFixture(deployContracts);

        await createTournament(6, "1000000000000000000", 0, 2);
        await expect(participate(6, [cardUncommon2Id, cardUncommon2Id], "1000000000000000000")).to.be.revertedWith("Tournament: duplicate cards");
    });

    it("Allow to participate to the same card twice", async () => {
        // Controversial test, but it's how it works now.
        const { createTournament, participate, cardUncommon1Id } = await loadFixture(deployContracts);
    ...
    it("Allow to give winner less on tournament end", async () => {
        const { createTournament, participate, payout, alice, cardBasic1Id } = await loadFixture(deployContracts);
    ...
    it("Does not allow to give winner more on tournament end", async () => {
        const { createTournament, participate, payout, alice, cardBasic1Id, instance } = await loadFixture(deployContracts);
        ...
        await expect(payout(9, [alice.address], ["1500000000000000000"])).to.be.revertedWith("Tournament: insufficient balance");
    ...
    it("Does not allow to give to someone else on tournament end", async () => {
        const { createTournament, participate, payout, instance, bob, cardBasic1Id } = await loadFixture(deployContracts);

        await createTournament(10, "1000000000000000000", 0, 1);
        await participate(10, [cardBasic1Id], "1000000000000000000");
        const contractBalanceBefore = await ethers.provider.getBalance(instance.address);
        await expect(payout(10, [bob.address], ["500000000000000000"])).to.be.revertedWith("Tournament: winner is not registered");
        const contractBalanceAfter = await ethers.provider.getBalance(instance.address);
        expect(contractBalanceAfter).to.be.equal(contractBalanceBefore);
    });
});
